...
export class SensorReadingViewComponent {
  displayedColumns: string[] = ['id', 'startTime', 'readingTime', 'value'];
  dataSource = new MatTableDataSource<SensorReadingModel>();

  @ViewChild(MatPaginator) paginator!: MatPaginator;

  name = "";

  constructor(private location: Location, private sensorService: SensorService) {
    this.name = (this.location.getState() as any).data;

    this.sensorService.sensorReading$.subscribe(x => {
